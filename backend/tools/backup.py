...
def main() -> int:
    ...
    if cmd == "create":
        ...
        # 4d：扫备份**内容**（attach 解开），不扫映像文件本身
        mnt = hdiutil(["attach", arg, "-nobrowse", "-readonly"], _passphrase())
        mount_point = [l.split()[-1] for l in mnt.stdout.splitlines()
                       if "/Volumes" in l][-1].strip()
        scan_dir = mount_point
        r = subprocess.run([sys.executable,
                            os.path.join(os.path.dirname(__file__), "secret_scan.py"), scan_dir],
                           capture_output=True, text=True)
        r2 = subprocess.run([sys.executable,
                             os.path.join(os.path.dirname(__file__), "data_ingress_scan.py"), scan_dir],
                            capture_output=True, text=True)
        hdiutil(["detach", mount_point])
